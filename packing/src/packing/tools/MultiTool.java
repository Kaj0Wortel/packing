...
public class MultiTool {
     ...
     * @param <B> class representing the input iterable class.
     * @param iterable the iterable to be converted.
     * @param classValue the class of the array to be created.
     * @param size the size of the array to be created.
     * @return an array containing all elements generated by the iterator,
     *     but within the provided bounds.
     */
    @SuppressWarnings("unchecked")
    public static <A, B extends A> A[] iterableToArray(
            Iterable<B> iterable, Class<A> classValue, int size) {
        if (iterable == null || classValue == null) return null;
        if (size < 0)
            throw new IllegalArgumentException("Negative array size!");
        
        A[] array = (A[]) Array.newInstance(classValue, size);
     ...
     * 
     * @param <A> class representing the output array class.
     * @param <B> class representing the input iterator class.
     * @param iterable the iterable to be converted.
     * @param classValue the class of the array to be created.
     * @param size the size of the array to be created.
     * @return an array containing all elements generated by the iterator,
     ...
     */
    @SuppressWarnings("unchecked")
    public static <A, B extends A> A[] iteratorToArray(
            Iterator<B> it, Class<A> classValue, int size) {
        if (it == null || classValue == null) return null;
        if (size < 0)
            throw new IllegalArgumentException("Negative array size!");
    ...
    /**
     * Checks whether the two given collections are fully disjoint.
     * 
     * @param <A> the used in the collections and comparator.
     * @param c1 the first collection.
     * @param c2 the second collection.
     * @param comp the comparator.
     * @return true iff the collections are disjoint.
     */
    public static <A> boolean disjoint(Collection<? extends A> c1,
            Collection<? extends A> c2, Comparator<A> comp) {
        Iterator<? extends A> it1 = c1.iterator();
        while (it1.hasNext()) {
            A elem1 = it1.next();
            
            Iterator<? extends A> it2 = c2.iterator();
            while (it2.hasNext()) {
                A elem2 = it2.next();
                
                if (comp.compare(elem1, elem2) == 0) return false;
            }
        }
